...
const Navbar = ({ onSelectCategory, onSearch }) => {
  ...
  const fetchData = async (value) => {
    try {
      const response = await axios.get("https://springboot-ecommerce-v1gh.onrender.com/api/products");
  ...
  const handleChange = async (value) => {
    ...
    try {
      const response = await axios.get(
        `https://springboot-ecommerce-v1gh.onrender.com/api/products/search?keyword=${value}`
      );
      setSearchResults(response.data);
